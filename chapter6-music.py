...
class MyWindow(glesutils.GameWindow):
    ...
    def on_frame(self, ftime):
        ...
        # calculate 500 frames for its square and average value as voice intensity
        scale_factor = 0
        if frame_position + 1000 < len(data):
            for i in range(1, 500):
                scale_factor1 = scale_factor + int.from_bytes(data[frame_position + 2 * i:frame_position + (2 * i) + 1],
...
# a module for processing voice
pygame.mixer.pre_init(44100, -16, 2, 2048)
pygame.mixer.init()
music = pygame.mixer.Sound("test.wav")
...
print("readframes")
# store voice in bytes format, voice encoding uses 2 bytes as a group
